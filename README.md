...
it is part of public HTTP paths and generated SDK identity.
...
## Native gRPC contract

Add gRPC metadata to the artifact's `pyproject.toml`:
...
id = "create_order"
task_type = "io"
entrypoint = "service:create_order"
...
`descriptor.pb` must be a protobuf `FileDescriptorSet` inside the revision ZIP.
Generate it with imports included, for example:
...
Clients use the ordinary generated `OrderServiceStub`. The fixed server-side
`GenericRpcHandler` resolves `/examples.orders.v1.OrderService/CreateOrder` to
the active revision and forwards the original protobuf bytes without an envelope
or an extra serialization pass.

The script entrypoint receives the decoded request message as its second
positional argument. It may return a response protobuf message, serialized
response bytes, or a mapping compatible with the declared response message:
...
The initial implementation supports unary-unary RPCs. Use a new protobuf package
or service version for breaking schema changes; compatible revisions may retain
the same method path.

## Generated Python SDK

When a revision declares gRPC endpoints, its ZIP must contain the original Proto
tree under `grpc_contract.proto_root` as well as `descriptor.pb`. Registration:

1. recompiles the Proto sources and verifies that they match `descriptor.pb`;
...
wheel. A changed schema must use a new `grpc_contract.version`; package versions
